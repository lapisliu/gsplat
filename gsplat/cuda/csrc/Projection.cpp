...
projection_4dgs_fused_fwd(
    ...
    conics_shape.append({C, N, 3});
    at::Tensor conics = at::empty(conics_shape, opt);
    at::DimVector weighted_opacities_shape(batch_dims);
    weighted_opacities_shape.append({C, N});
    at::Tensor weighted_opacities = at::zeros(weighted_opacities_shape, opt);
    at::Tensor compensations;
    if (calc_compensations) {
        // we dont want NaN to appear in this tensor, so we zero intialize it
        at::DimVector compensations_shape(batch_dims);
    ...
    launch_projection_4dgs_fused_fwd_kernel(
        // inputs
        means,
        covars,
        quats,
    ...
    return std::make_tuple(radii, means2d, depths, conics, weighted_opacities, compensations);
}
...
projection_4dgs_fused_bwd(
    ...
    CHECK_INPUT(v_depths);
    CHECK_INPUT(v_conics);
    CHECK_INPUT(v_weighted_opacities);
    if (compensations.has_value()) {
        CHECK_INPUT(compensations.value());
    }
    if (v_compensations.has_value()) {
    ...
    at::Tensor v_quats_t = at::zeros_like(quats_t);
    at::Tensor v_scales_t = at::zeros_like(scales_t);
    at::Tensor v_viewmats;
    if (viewmats_requires_grad) {
        v_viewmats = at::zeros_like(viewmats);
    }

    launch_projection_4dgs_fused_bwd_kernel(
        // inputs
        means,
        covars,
        quats,
    ...
    return std::make_tuple(v_means, v_covars, v_quats, v_scales, v_opacities, v_ts, v_quats_t, v_scales_t, v_viewmats);
}
